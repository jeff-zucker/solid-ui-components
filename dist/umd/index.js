(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["exports", "./user.js", "./model/rss.js", "./model/sparql.js", "./view/accordion.js", "./view/app.js", "./view/form.js", "./view/menu.js", "./view/modal.js", "./view/selectorPanel.js", "./view/table.js", "./view/tabs.js"], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require("./user.js"), require("./model/rss.js"), require("./model/sparql.js"), require("./view/accordion.js"), require("./view/app.js"), require("./view/form.js"), require("./view/menu.js"), require("./view/modal.js"), require("./view/selectorPanel.js"), require("./view/table.js"), require("./view/tabs.js"));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.user, global.rss, global.sparql, global.accordion, global.app, global.form, global.menu, global.modal, global.selectorPanel, global.table, global.tabs);
    global.index = mod.exports;
  }
})(typeof globalThis !== "undefined" ? globalThis : typeof self !== "undefined" ? self : this, function (_exports, user, _rss, _sparql, _accordion, _app, _form, _menu, _modal, _selectorPanel, _table, _tabs) {
  "use strict";
  ...
  user = _interopRequireWildcard(user);
  ...
  // DataSources
  // Templates
  const sparql = new _sparql.Sparql();
  ...
  class SolidUIcomponent {
    ...
    async processComponent(element, subject, json) {
      ...
      if (!json) {
        console.log("No ComponentHASH ", subject);
        return;
      } // default color,orientation,position


      json = this.getDefaults(json); //    if(json.type.match(/Link/)){
      ...
      if (json.type === 'App') {
        return await new _app.App().render(this, json);
      } else if (json.type && json.type.match(/Link/)) {
        return await window.displayLink(element, json, element);
      } else if (json.type === 'Menu') {
        return await new _menu.Menu().render(this, json, element);
      } else if (json.type === 'Feed' && (json.href || json.link)) {
        return await new _rss.Feed().render(this, json);
      } else if (json.type === 'SelectorPanel') {
        let panel = new _selectorPanel.SelectorPanel();
        let d = this.setDefaults(json);
        return await panel.render(json);
      } else if (json.type === 'ModalButton') {
        return await new _modal.Modal().render(this, json);
      } else if (json.type === 'Accordion') {
        return await new _accordion.Accordion().render(this, json);
      } else if (json.type === 'FormComponent') {
        return await new _form.Form().render(this, json);
      } else if (json.type === 'Tabset') {
        return await new _tabs.Tabs().render(this, json);
      }

      let contentWrapper = document.createElement('DIV');
      let results, before, after, content, label; // RECORD
      ...
      this.log('Parts for Component', results); // TEMPLATE
      ...
      if (template == "Table") {
        let compo = await new _table.Table().render(json);
        if (compo) contentWrapper.appendChild(compo);
        return contentWrapper;
      }
      ...
      let recurring; // let template = json.template.value ?json.template.value :json.template ;
      ...
      */
      ...
      after = template.after; //        recurring = recurring || template;
    ...
    async getComponentHash(subject, hash) {
      ...
      for (let p of predicatePhrases) {
        ...
        if (obj.termType === "BlankNode") {
          ...
          if (!hash[pred]) hash[pred] = obj;else if (typeof hash[pred] != 'ARRAY') hash[pred] = [obj];else hash[pred].push(obj);
        } else if (obj.termType === "Collection") {
          obj = obj.elements;

          for (let uri of obj) {
            let component = await this.getComponentHash(uri, {});
            if (!hash[pred]) hash[pred] = [component];else hash[pred].push(component);
          }
        } else {
    ...
    async sparqlQuery(endpoint, queryString, json) {
      if (typeof Comunica != "undefined") return await sparql.comunicaQuery(endpoint, queryString, json);else return await sparql.rdflibQuery(solidUI, kb, endpoint, queryString, json);
    }
    /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      DISPLAY METHODS
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
    ...
    createElement(elementType, className, html, styles) {
      ...
      if (styles) {
        for (let s of styles.split(/;/)) {
          let pair = s.split(/:/);
          if (pair.length == 2) element.style[pair[0].trim()] = pair[1].trim();
        }
      }
    ...
    async loadUnlessLoaded(uri) {
      try {
        ...
        if (!kb.any(null, null, null, graph)) {
          ...
          if (kb.any(null, null, null, graph)) console.log(`<${graph.uri}> loaded!`);else console.log(`<${graph.uri}> could not be loaded!`); //console.log(kb.match(null,null,null,graph));
