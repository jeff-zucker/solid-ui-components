(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["./databrowser.js", "./model/rss.js"], factory);
  } else if (typeof exports !== "undefined") {
    factory(require("./databrowser.js"), require("./model/rss.js"));
  } else {
    var mod = {
      exports: {}
    };
    factory(global.databrowser, global.rss);
    global.user = mod.exports;
  }
})(typeof globalThis !== "undefined" ? globalThis : typeof self !== "undefined" ? self : this, function (_databrowser, _rss) {
  "use strict";
  ...
  function showByClass(event) {
    ...
    for (let element of elements) {
      ...
      if (element.getAttribute('value').indexOf(wanted) > -1) element.style.display = "block"; //  if(element.classList.contains(wanted)) element.style.display="block";
  ...
  function highlightMenuSelection(e) {
    ...
    for (let sib of siblings) {
      if (sib.dataset.link === wanted) sib.classList.add('selected');else sib.classList.remove('selected');
    }
  }

  function toggleAccordion(e) {
    let wanted = e ? e.target.dataset.link : null;
    let siblings = e.target.closest('UL').children; // HIDE ALL ACCORDION DROPDOWNS
  ...
  window.displayLink = async function (e, item, element) {
    ...
    item.type = item.type.replace(/http:\/\/www.w3.org\/ns\/ui#/, ''); // IF POPOUT, DISPLAY IN ITS OWN OVERLAY AND RETURN
    ...
    if (item.type.startsWith("SolidOS")) {
      ...
      if (item.pane) {
        ...
        let me = await (0, _databrowser.applyProfile)(null, item.pod, 'podLoad');
        subject = me.storages[0];
      }

      subject = window.kb.sym(subject);
      (0, _databrowser.setHistory)(subject.uri);
    ...
    if (tabulator) tabulator.style.display = "none"; // FIND CONTAINING ELEMENT
    ...
    if (content) {
      ...
      span.innerHTML = content; //      span = mungeAnchors(span);
    ...
    if (item.type === "Feed" || item.linkType === "Feed") {
      let content = await new _rss.Feed().render(solidUI, item);
      /*
            let newItem = {
              type:"App",
      ...
      */
      ...
      if (content) {
        ...
        content = mungeAnchors(content); //        containingEl.innerHTML=""

        return content;
      } else if (item.href) {
        let content = await getIframe(item, containingEl, item.nativeIframe); //        containingEl.innerHTML=""
  ...
  async function getIframe(item, containingEl, nativeIframe) {
    ...
    if (nativeIframe) {
      ...
      try {
        ...
        if (content) {
          content = await content.text(); //        if(!content.match(/^\s*</)) return await goto(uri,containingEl);
