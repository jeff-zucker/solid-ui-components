(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["exports", "./model/sparql.js", "./view/table.js", "./view/form.js", "../node_modules/mime/"], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require("./model/sparql.js"), require("./view/table.js"), require("./view/form.js"), require("../node_modules/mime/"));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.sparql, global.table, global.form, global.mime);
    global.utils = mod.exports;
  }
})(typeof globalThis !== "undefined" ? globalThis : typeof self !== "undefined" ? self : this, function (_exports, _sparql, _table, _form, _mime) {
  "use strict";
  ...
  _exports.CU = void 0;
  _mime = _interopRequireDefault(_mime);
  ...
  */
  ...
  class CU {
    ...
    getObjects(subject, predicate) {
      return UI.store.each(subject, predicate).map(s => {
        return s.value;
      });
    }
    ...
    async show(type, uri, string, targetSelector, forceReload) {
      ...
      type = type.replace(/.*\//, '');
      if (this._show[type]) return await this._show[type](uri, string, targetSelector, forceReload);
    }
    ...
    _show = {
      ...
      markdown: async (uri, string, targetSelector) => {
        string ||= (await this.loadFile(uri)).body; // string ||= await this.loadFile(uri);
      ...
      Form: async (subject, targetSelector, forceReload) => {
        ...
        const form = await (await new _form.Form()).render({
          form: subject.value
        });

        if (targetSelector) {
          document.querySelector(targetSelector).innerHTML = "";
          document.querySelector(targetSelector).appendChild(form); // return await this.showIframeSrcDoc( form.outerHTML, targetSelector );
      ...
      SparqlQuery: async (subject, targetSelector, forceReload) => {
        ...
        let table = new _table.Table().render({
          parts: await new _sparql.Sparql().sparqlQuery(e, q, forceReload),
          cleanNodes: true,
          sortOn: 'term'
        });
      ...
      DataTemplate: async (subject, targetSelector, store) => {
        ...
        let data = await new _sparql.Sparql().sparqlQuery(endpoint, query, store);
        let interpolated = "";
      ...
      PageDefinition: async (subject, targetSelector) => {
        ...
        let dom = DM.parseFromString(templateInfo.body, "text/html"); //alert( XS.serializeToString(dom) );
        ...
        for (let p of parts) {
          ...
          */
          ...
          if (i && label) dom.querySelector('#' + label).appendChild(await this.show(i.contentType, url));
        }
    ...
    async dataTemplate2Iframe(mainSubject, targetSelector) {
      ...
      let data = await new _sparql.Sparql().sparqlQuery(endpoint, query, UI.store);
      let interpolated = "";
    ...
    async template2Iframe(mainSubject, targetSelector) {
      ...
      let beforeData = await new _sparql.Sparql().sparqlQuery(beforeEndpoint, beforeQuery, UI.store);
    ...
    showIframeSrc(src, targetSelector) {
      ...
      iframe.src = src;
      if (targetSelector) this.show_iframe(iframe, targetSelector);else return iframe;
    }
    ...
    makeSelector(options, onchange, selected, targetSelector, size) {
      ...
      for (let option of options) {
        let value, label;
        if (typeof option === "string") value = label = option;else {
          value = option[0];
          label = option[1];
        }
        ...
        label ||= "/";
        let optionEl = document.createElement('OPTION'); //      optionEl.innerHTML = option.replace(/http[s]*:\/\/[^\/]*\//,'') || option;
    ...
    fileInfo(url) {
      ...
      */
      ...
      contentType = isContainer ? "text/turtle" : _mime.default.getType(url) || "unknown";
      ...
      return {
        ...
        editable,
        type,
        // markdown, graphviz, html, rdf ...
        contentType,
        // text/vnd.graphviz, text/turtle ...
        host: newu.origin + "/",
    ...
    async crossLoad(uri, string, forceReload) {
      if (forceReload) {//     UI.store.removeDocument(UI.rdf.sym(uri));
      }

      if (string) UI.rdf.parse(string, UI.store, uri, 'text/turtle');else await UI.store.fetcher.load(uri);
