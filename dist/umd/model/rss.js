(function (global, factory) {
  ...
  class Feed {
    async fetchAndParse(feedUri, proxy) {
      ...
      let feedDom = domParser.parseFromString(feedContent, "text/xml"); // find items (RSS) or entries (Atom)
      //

      let items = feedDom.querySelectorAll("item") || null;
      items = items.length < 1 ? feedDom.querySelectorAll("entry") : items; // parse items
      //
      ...
      items.forEach(el => {
        ...
        //
        let link = el.querySelector("link").innerHTML; // vox

        if (!link) link = el.querySelector('link').getAttribute('href'); // reddit
        ...
        if (!link.match(/^http/)) link = link.replace(/.*\[CDATA\[/, ''); // always use https, not http

        link = link.replace(/^http:/, 'https:'); // get the title
